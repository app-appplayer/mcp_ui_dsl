...
|---|---|---|---|---|
| `accessibleWrapper` | utility | Core | — | — |
| `alertDialog` | dialog | Core | v1.0 | `alert` |
| `align` | layout | Core | — | — |
| `animatedAlign` | animation | Core | v1.3 | — |
| `animatedContainer` | animation | Core | — | — |
| `animatedDefaultTextStyle` | animation | Core | v1.3 | — |
| `animatedOpacity` | animation | Core | v1.3 | — |
| `animatedPositioned` | animation | Core | v1.3 | — |
| `aspectRatio` | layout | Core | — | — |
| `avatar` | display | Core | — | — |
| `badge` | display | Core | — | — |
| `banner` | display | Core | — | — |
...
| `decoration` | display | Core | — | — |
| `divider` | display | Core | — | — |
| `dragTarget` | interaction | Core | — | — |
| `draggable` | interaction | Core | — | — |
| `drawer` | navigation | Core | — | — |
...
| `floatingActionButton` | navigation | Core | — | — |
| `flow` | utility | Core | — | — |
| `form` | input | Core | — | — |
...
| `image` | display | Core | — | — |
| `imageFilter` | display | Core | v1.3 | — |
| `indexedStack` | layout | Core | — | — |
| `inkWell` | interaction | Core | — | — |
...
| `numberField` | input | Core | — | — |
| `numberStepper` | input | Core | — | — |
| `offlineFallback` | utility | Core | — | — |
| `opacity` | animation | Core | v1.3 | — |
| `padding` | layout | Core | — | — |
| `pageView` | scroll | Core | — | — |
| `permissionPrompt` | utility | Core | — | — |
| `placeholder` | display | Core | — | — |
| `popupMenuButton` | navigation | Core | — | — |
...
| `rangeSlider` | input | Core | — | — |
| `rating` | input | Core | — | — |
| `richText` | display | Core | — | — |
| `rive` | animation | Core | v1.3 | — |
| `safeArea` | layout | Core | — | — |
| `scrollAnimated` | animation | Core | v1.3 | — |
| `scrollBar` | scroll | Core | — | — |
| `scrollView` | scroll | Core | — | — |
...
| `slider` | input | Core | — | — |
| `snackBar` | dialog | Core | — | — |
| `spacer` | layout | Core | — | — |
| `stack` | layout | Core | — | — |
| `staggeredGrid` | list | Core | v1.3 | — |
| `stepper` | input | Core | — | — |
...
Cancel / Confirm pair with a string body.
...
Legacy `type: alert` accepted as alias for `alertDialog`.
...
#### `unknown_variant_is_ok_for_now`

Phase 1 does not enforce `variant` enum on action entries; nested shape is permissive.
...
#### `wrong_type` — **expected: validation_error** at `#/oneOf`

Root `type` must be `alertDialog` or alias.
...
#### `slide_in_from_top_start`

```json
{
  "type": "animatedAlign",
  "alignment": "{{ready ? 'center' : 'topStart'}}",
  "duration": 400,
  "child": { "type": "icon", "icon": "check_circle" }
}
```
...
| `margin` | `EdgeInsets` | no | — | Animated outer margin. |
| `alignment` | `Alignment` | no | — | Animated child alignment. |
...
```json
{
  ...
  "children": [
    { "type": "image", "src": "{{cover}}" },
    {
      ...
      "duration": 400,
      "curve": "emphasized",
      "child": { "type": "iconButton", "icon": "favorite" }
    }
  ]
}
```
...
| `click` | `Action` | no | — | Action fired when the widget is tapped. Runtime wraps the widget in a gesture surface and dispatches the action on tap. Widget-local activation surfaces (`button.onTap`, `iconButton.onTap`, `richText.spans[].onTap`, …) remain canonical for those widgets and are NOT replaced by `click`; `click` is the universal fallback for widgets that have no dedicated activation slot.  |
| `tooltip` | `string` | no | — | Hover / long-press tooltip text. Runtime wraps the widget in a `Tooltip` surface.  |
| `aspectRatio` | `number` | no | 1.0 | Width-to-height ratio (e.g., `1.5` for 3:2). Defaults to `1.0` (square) when omitted. |
...
## `bottomNavigation`  *(navigation · Core)*
...
- kind: `single` · key: `child`

### Examples

#### `sized_decorated_no_child`

Sized + decorated + padded, without a child. Validates via schema alone.
...
Child is another `box`. Self-contained within the Phase 1 registry.
...
#### `unknown_type` — **expected: validation_error** at `#/oneOf`

`type` value must be `box` or a declared alias.
...
| `options.responsive` | `boolean` | no | true | Resize with container |
| `options.animation.duration` | `number` | no | 1000 | Animation duration (ms) |
...
| `default` | `Widget` | no | — | Rendered when no case matches (switch form). |
...
| `click` | `Action` | no | — | Action fired when the widget is tapped. Runtime wraps the widget in a gesture surface and dispatches the action on tap. Widget-local activation surfaces (`button.onTap`, `iconButton.onTap`, `richText.spans[].onTap`, …) remain canonical for those widgets and are NOT replaced by `click`; `click` is the universal fallback for widgets that have no dedicated activation slot.  |
| `tooltip` | `string` | no | — | Hover / long-press tooltip text. Runtime wraps the widget in a `Tooltip` surface.  |
| `content` | `Widget` | no | — | Dashboard tile content tree. When omitted the tile renders empty (fallback). |
...
| `showGrid` | `boolean` | no | true | Draw background grid. |
| `showLabels` | `boolean` | no | true | Draw axis labels. |
| `lineColor` | `Color` | no | #2196F3 | Line / bar color. |
...
| `click` | `Action` | no | — | Action fired when the widget is tapped. Runtime wraps the widget in a gesture surface and dispatches the action on tap. Widget-local activation surfaces (`button.onTap`, `iconButton.onTap`, `richText.spans[].onTap`, …) remain canonical for those widgets and are NOT replaced by `click`; `click` is the universal fallback for widgets that have no dedicated activation slot.  |
| `tooltip` | `string` | no | — | Hover / long-press tooltip text. Runtime wraps the widget in a `Tooltip` surface.  |
| `tag` | `string` | yes | — | Identifier shared between source and destination pages. Both sides MUST set the same tag for the morph to engage. Tags must be unique within each page (a single page cannot have two heroes with the same tag).  |
| `child` | `Widget` | yes | — | The widget that morphs across the route boundary. |
| `transitionOnUserGestures` | `boolean` | no | false | When true, also morphs during gesture-driven back navigation (iOS swipe-from-edge). Adds slight render cost.  |
| `flightShuttleBuilder` | `Widget` | no | — | Optional intermediate widget used during the morph itself. When omitted the runtime tweens between the source and destination children.  |
...
| `click` | `Action` | no | — | Action fired when the widget is tapped. Runtime wraps the widget in a gesture surface and dispatches the action on tap. Widget-local activation surfaces (`button.onTap`, `iconButton.onTap`, `richText.spans[].onTap`, …) remain canonical for those widgets and are NOT replaced by `click`; `click` is the universal fallback for widgets that have no dedicated activation slot.  |
| `tooltip` | `string` | no | — | Hover / long-press tooltip text. Runtime wraps the widget in a `Tooltip` surface.  |
| `icon` | `string | object` | yes | — | Name, URL, bundle path, data URI, or codepoint object — see the five forms documented in the widget summary.  |
...
| `color` | `Color` | no | — | Icon color. Applied to named / codepoint / SVG forms; for raster `URL` form it tints when supported. Ignored when `shader` is set.  |
| `shader` | `Gradient` | no | — | Gradient/shader fill (linear / radial / sweep). Painted via a shader mask over the icon's glyph alpha. Mutually exclusive with `color` — when both are present `shader` wins.  |
...
Icon painted with a linear gradient instead of a solid color.
...
| `click` | `Action` | no | — | Action fired when the widget is tapped. Runtime wraps the widget in a gesture surface and dispatches the action on tap. Widget-local activation surfaces (`button.onTap`, `iconButton.onTap`, `richText.spans[].onTap`, …) remain canonical for those widgets and are NOT replaced by `click`; `click` is the universal fallback for widgets that have no dedicated activation slot.  |
| `tooltip` | `string` | no | — | Hover / long-press tooltip text. Runtime wraps the widget in a `Tooltip` surface.  |
| `src` *(aliases: `source`, `backgroundImage`)* | `string` | yes | — | Image URL, `bundle://...`, or `client://...`. |
...
| `click` | `Action` | no | — | Action fired when the widget is tapped. Runtime wraps the widget in a gesture surface and dispatches the action on tap. Widget-local activation surfaces (`button.onTap`, `iconButton.onTap`, `richText.spans[].onTap`, …) remain canonical for those widgets and are NOT replaced by `click`; `click` is the universal fallback for widgets that have no dedicated activation slot.  |
| `tooltip` | `string` | no | — | Hover / long-press tooltip text. Runtime wraps the widget in a `Tooltip` surface.  |
| `filter` | `string` | yes | — | Filter kind: - `sepia` — warm-tone tint mimicking aged photograph. - `grayscale` — desaturate to luminance. - `blur` — Gaussian blur (sigma controlled by `intensity`). - `saturation` — multiply chroma by `intensity` (0 = grayscale, 1 = unchanged, >1 = pop). - `brightness` — multiply luminance. - `contrast` — increase / decrease contrast. - `invert` — colour invert (negative).  |
| `intensity` | `number` | no | 1.0 | Filter strength. Range and default depend on `filter`: - `sepia` / `grayscale` / `invert` — `0.0` (off) .. `1.0` (full). Default `1.0`. - `blur` — sigma in pixels (`0.0` off, typical 4..16). - `saturation` / `brightness` / `contrast` — multiplier (`1.0` unchanged).  |
...
| `click` | `Action` | no | — | Action fired when the widget is tapped. Runtime wraps the widget in a gesture surface and dispatches the action on tap. Widget-local activation surfaces (`button.onTap`, `iconButton.onTap`, `richText.spans[].onTap`, …) remain canonical for those widgets and are NOT replaced by `click`; `click` is the universal fallback for widgets that have no dedicated activation slot.  |
| `tooltip` | `string` | no | — | Hover / long-press tooltip text. Runtime wraps the widget in a `Tooltip` surface.  |
| `images` | `array<AssetRef>` | yes | — | Image asset list. Each entry is an AssetRef. |
| `initialIndex` | `number` | no | 0 | Index of the image shown first. |
...
| `click` | `Action` | no | — | Action fired when the widget is tapped. Runtime wraps the widget in a gesture surface and dispatches the action on tap. Widget-local activation surfaces (`button.onTap`, `iconButton.onTap`, `richText.spans[].onTap`, …) remain canonical for those widgets and are NOT replaced by `click`; `click` is the universal fallback for widgets that have no dedicated activation slot.  |
| `tooltip` | `string` | no | — | Hover / long-press tooltip text. Runtime wraps the widget in a `Tooltip` surface.  |
| `margin` *(aliases: `value`)* | `EdgeInsets` | yes | — | Outer margin. |
| `child` | `Widget` | yes | — | Child widget. |
...
| `volume` | `number` | no | 1.0 | Volume 0.0–1.0 |
| `controls` | `boolean` | no | true | Show native controls |
| `poster` | `AssetRef` | no | — | Preview image rendered before playback starts (video only). |
| `waveform` | `boolean` | no | false | Audio mode only. When true, the audio's amplitude waveform is rendered above the transport controls, advancing with playback. Ignored for video sources.  |
| `width` | `number` | no | — | Display width |
| `height` | `number` | no | — | Display height |
| `onPlay` | `Action` | no | — | Fired when playback starts |
| `onPause` | `Action` | no | — | Fired when playback pauses |
...
| `click` | `Action` | no | — | Action fired when the widget is tapped. Runtime wraps the widget in a gesture surface and dispatches the action on tap. Widget-local activation surfaces (`button.onTap`, `iconButton.onTap`, `richText.spans[].onTap`, …) remain canonical for those widgets and are NOT replaced by `click`; `click` is the universal fallback for widgets that have no dedicated activation slot.  |
| `tooltip` | `string` | no | — | Hover / long-press tooltip text. Runtime wraps the widget in a `Tooltip` surface.  |
| `condition` | `object | binding` | no | — | Boolean expression (string) or constraint object like `{minWidth, maxWidth}`; selects `then` when truthy. |
...
#### `text_run_with_emphasis`

Simple multi-style text run.
...
TextSpan + WidgetSpan mixed (inline icon).
...
| `click` | `Action` | no | — | Action fired when the widget is tapped. Runtime wraps the widget in a gesture surface and dispatches the action on tap. Widget-local activation surfaces (`button.onTap`, `iconButton.onTap`, `richText.spans[].onTap`, …) remain canonical for those widgets and are NOT replaced by `click`; `click` is the universal fallback for widgets that have no dedicated activation slot.  |
| `tooltip` | `string` | no | — | Hover / long-press tooltip text. Runtime wraps the widget in a `Tooltip` surface.  |
| `src` | `AssetRef` | yes | — | Rive asset path (`bundle://...riv`, `https://...riv`, `assets/...riv`). |
| `artboard` | `string` | no | — | Named artboard to render. Defaults to the file's primary artboard. |
...
| `click` | `Action` | no | — | Action fired when the widget is tapped. Runtime wraps the widget in a gesture surface and dispatches the action on tap. Widget-local activation surfaces (`button.onTap`, `iconButton.onTap`, `richText.spans[].onTap`, …) remain canonical for those widgets and are NOT replaced by `click`; `click` is the universal fallback for widgets that have no dedicated activation slot.  |
| `tooltip` | `string` | no | — | Hover / long-press tooltip text. Runtime wraps the widget in a `Tooltip` surface.  |
| `bindings` | `array<object>` | yes | — | Per-property scroll bindings. Each entry is `{property, fromOffset, toOffset, fromValue, toValue, curve?}`: - `property` — `opacity` / `scale` / `translateX` /   `translateY` / `rotate`. Other properties may be wired by   runtime extensions. - `fromOffset` / `toOffset` — scroll-position window in   logical pixels. - `fromValue` / `toValue` — interpolated target values for the   property at the window endpoints. - `curve` — optional easing across the window. Defaults to   `linear`.  |
...
#### `sliver_mode_collapsing_app_bar`
...
| `click` | `Action` | no | — | Action fired when the widget is tapped. Runtime wraps the widget in a gesture surface and dispatches the action on tap. Widget-local activation surfaces (`button.onTap`, `iconButton.onTap`, `richText.spans[].onTap`, …) remain canonical for those widgets and are NOT replaced by `click`; `click` is the universal fallback for widgets that have no dedicated activation slot.  |
| `tooltip` | `string` | no | — | Hover / long-press tooltip text. Runtime wraps the widget in a `Tooltip` surface.  |
| `binding` | `string` | no | — | State path bound two-way to the input value. |
...
| `obscureText` | `boolean` | no | false | Hide input characters (passwords). |
| `readOnly` | `boolean` | no | false | Whether the field is read-only. |
| `maxLines` | `integer` | no | 1 | Maximum number of visible lines. |
| `maxLength` | `integer` | no | — | Maximum character length enforced at input time. |
| `inputType` | `string` | no | text | Virtual keyboard hint / input semantics. |
| `validation` | `ValidationConfig` | no | — | Input constraints; see `../../07_Security.md` §7.2.1. |

### Events

| Name | Payload | Aliases |
|---|---|---|
| `onChange` | `value: string`, `type: change` | `onChanged`, `change` |
| `onSubmit` | `value: string`, `type: submit` | `submit` |
| `onFocus` | `type: focus` | — |
| `onBlur` | `value: string`, `type: blur` | `blur` |

### Examples

#### `binding_only`

Canonical binding shorthand — runtime hydrates + writes back automatically.
...
#### `explicit_value_onChange`

Opts out of binding shorthand — explicit `value` + `onChange`.
...
#### `missing_type` — **expected: validation_error** at `#/required/type`

`type` missing — top-level validation error.
...
#### `bad_input_type` — **expected: validation_error** at `#/properties/inputType/enum`
...
| `click` | `Action` | no | — | Action fired when the widget is tapped. Runtime wraps the widget in a gesture surface and dispatches the action on tap. Widget-local activation surfaces (`button.onTap`, `iconButton.onTap`, `richText.spans[].onTap`, …) remain canonical for those widgets and are NOT replaced by `click`; `click` is the universal fallback for widgets that have no dedicated activation slot.  |
| `tooltip` | `string` | no | — | Hover / long-press tooltip text. Runtime wraps the widget in a `Tooltip` surface.  |
| `data` | `array | binding` | yes | — | Hierarchical data; each node MAY carry a `children` array. Literal array or binding. |
| `childrenKey` | `string` | no | children | Field name of the children array |
| `indentation` | `number` | no | 24 | Per-depth horizontal indentation in logical pixels. |
| `itemPadding` | `EdgeInsets` | no | — | Padding applied inside every tree row (expandable and leaf). Defaults to `{vertical: 4, right: 8}`. Adjust the vertical component to change row height / density. |
| `itemTemplate` | `Widget` | no | — | Template rendered per node; `{{item}}` is node data. Falls back to a default list-item renderer when omitted. |
| `expandable` | `boolean` | no | true | Allow expand/collapse |
| `initiallyExpanded` | `boolean` | no | false | Expand all nodes on mount |
| `selectable` | `boolean` | no | false | Whether a node can be selected by tap. |
| `showLines` | `boolean` | no | true | Draw connector lines between parent and leaf nodes. |
| `selectedColor` | `Color` | no | — | Background color of the currently selected row. |
...
| `click` | `Action` | no | — | Action fired when the widget is tapped. Runtime wraps the widget in a gesture surface and dispatches the action on tap. Widget-local activation surfaces (`button.onTap`, `iconButton.onTap`, `richText.spans[].onTap`, …) remain canonical for those widgets and are NOT replaced by `click`; `click` is the universal fallback for widgets that have no dedicated activation slot.  |
| `tooltip` | `string` | no | — | Hover / long-press tooltip text. Runtime wraps the widget in a `Tooltip` surface.  |
| `url` | `string` | no | null (if html set) | URL to load |
| `html` | `string` | no | null (if url set) | Raw HTML content — alternative to `url` |
| `allowNavigation` | `boolean` | no | true | Allow link navigation within the view |
| `enableJavaScript` | `boolean` | no | true | Enable JavaScript |
| `enableZoom` | `boolean` | no | true | Allow pinch-to-zoom |
| `width` | `number` | no | — | Widget width |
| `height` | `number` | no | — | Widget height |
